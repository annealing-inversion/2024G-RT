...
impl Quad {
    ...
    pub fn boxx(a: Vec3, b:Vec3, mat: Arc<dyn Material + Send + Sync>) -> Arc<HittableList> {
        ...
        sides.add(Arc::new(Quad::new(Vec3::new(max.x, min.y, max.z), dz * -1.0, dy, mat.clone())));
        sides.add(Arc::new(Quad::new(Vec3::new(max.x, min.y, min.z), dx * -1.0, dy, mat.clone())));
        sides.add(Arc::new(Quad::new(Vec3::new(min.x, min.y, min.z), dz, dy, mat.clone())));
        sides.add(Arc::new(Quad::new(Vec3::new(min.x, max.y, max.z), dx, dz * -1.0, mat.clone())));
